<template>
  <div
    ...
    :class="[{ 'multiselect--active': isOpen, 'multiselect--disabled': disabled, 'multiselect--above': isAbove }, {'is-invalid': state === false}]"
    @focus="activate"
    @click="onClick"
  ...
  >
    ...
    <div
      ...
      :style="{ minWidth: !width || !cellInput || !isOpen ? 'auto' : width + 'px' }"
    >
      <slot
        name="selection"
        :search="search"
    ...
    <transition name="multiselect">
      <div
        ...
        tabindex="-1"
        @mousedown.prevent
        :style="{ maxHeight: optimizedHeight + 'px', maxWidth: !optimizedWidth ? 'auto' : optimizedWidth + 'px', top: positionTop, left: positionLeft }"
        ref="list"
      >
          ...
          <template v-if="!max || internalValue.length < max">
            ...
            >
              <span
                ...
                @click.stop="select(option, $event)"
              ...
              >
                <!-- org => select(option) Modify By SoMRuk-->
...
.grid .table .multiselect__content-wrapper {
    position: fixed;
    display: block;
}
