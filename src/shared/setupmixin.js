import { status, configMode, cloneCostMode } from '@/shared/define'
...
export default {
  data() {
    return {
      ...
      cloneCostMode: { lists: cloneCostMode, current: cloneCostMode.filter(v => v.value == 'Y'), selected: [cloneCostMode[0]] }
    }
  },
  computed: {
    config() {
  ...
  methods: {
    ...
    GetCloneCost(val) {
      return GetObjVal(this.cloneCostMode.lists.find(d => d.value === val), 'text')
    },
    Saved() {
    ...
    SetErrorMsg(res, fields, gendata) {
      ...
      if (Array.isArray(res)) {
        res.forEach((d, i) => {
          ...
          if (d._result != null && d._result._status == 'F') {
